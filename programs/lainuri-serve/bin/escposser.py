...
hsk = lainuri.hs_k33.HSK_Printer()
def test_page(arg):
  hsk.set_print_concentration(**arg)
  pprint(hsk.escpos_printer.text(str(arg)))
  pprint(hsk.escpos_printer.qr("https://www.hypernova.fi/lainuri-self-checkout-machine/",
                              native=True,
                              size=3))
  time.sleep(1)
...
png_bytes = lainuri.printer.print_check_in_receipt(items=[])
hsk.escpos_printer.image('/tmp/weasy.png', impl=u'bitImageRaster')
time.sleep(1)
...
"""
args = {'max_printing_dots': 128, 'heating_time': 80, 'heating_interval': 32}
test_page(args)
